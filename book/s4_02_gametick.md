...
| 2 | hourly_parallel | countryHourlyUpdateOnlyChangeSafePrivateAndCache; 段内含 **CGameState::DoTradeRoutesUpdate** (sub_1401D9890, 资源输送路线 — 结构见 §4.21.1b) |
| 3 | HourlyCountryComponentUpdate | 串行逐国六 pass: ① sub_1406FDEA0 theatre 小时推进+失效移除 ("Removing Theater %s from %s" country.cpp:4649; cc+360 战区指针数组, 归属 tag 无存活国 → swap-delete); ② sub_1406FD710 cc+5136 关注 tag 表失效清理 (token 14346 war_relation, 对端 +5160/+5184 并行表双向摘除); ③ sub_140F080F0(gs+784) 段级一次性 — 语料空洞 (theatre.cpp 编译区, 推定战区管理器国家级整体刷新); ④ sub_1406FDE80 **CProductionStatus (cc+3944) 生产小时推进** (RTDynamicCast CGeneralProductionLine→CBuildingProductionLine, 不可建取消 + 联动对方国); ⑤ sub_1406FE1A0 capital 完整性检查 ("<TAG> has NO capital defined." → terminate; 失守 → sub_1406D88A0 选都 + 140710DD0 迁都); ⑥ sub_1406FD800 多组件复合 = **country.delayed_events** (cc+4752 数组, +196 倒计时到点 140A0F6F0 发射) → **country.calc_modifier** (MODIFIER_DECISION/MISSION_PREFIX) → **decision.hourly** (cc+4000) → cc+4024 装备市场 → cc+808 人力缓存 → cc+4032 情报机构 → cc+5544 **间谍行动 hourly** (operation_rescue_general 族) → cc+4080 **顾问槽刷新** ("Removing an assigned advisor that is not in the list of characters") → cc+4744 懒缓存重算 → **错峰日步 `hour == tag%24` 时 cc+5632 = DEFAULT_COASTAL_PROTECTION_STABILITY × 同阵营占比** (defines_game.h:70 NCountry) |
| 4 | (army 并行段) | 扁平全师数组并行: **army+1080 师级 attrition 缓存小时重算** (§4.18.14) + gs+1032 二维查询表小时重建 + 舰队第二遍并行; **hourlyUpdateUnits** = 占领 bundle 包夹 (EED690 ++ / EF2F30 −−, 计数归零 → profiler "theatremanager.endbundle" 复位 g_OccupationBundleConquer/Relation, theatre.cpp:5560/5572) + 单位容器步进 + **CCountryFuelStatus 燃料小时结算** (cc+5504); **army 错峰日步 sub_140C89090: `((army+28 师 id)+1) % 24 == hour` → army+1584 leader 旗门 → 指挥链聚合对象 `*(*(army+192)+136)` 重建** (清 8 统计容器 +1816/+2008/+2200/+2392/+2968/+3160/+2584/+2776, 从 +3528 数组四组数据重灌, 虚槽 [28]/[30]; 聚合对象类身份未决) — 每师每天恰一次 |
| 5 | hourlyUpdateUnits | (并入阶段 4 描述) |
| 6 | postHourlyUpdate (sub_140705630) | 串行逐活跃国: ① sub_1406FF1F0 批量重算本小时累积修改器后 **cc+4320 清零** (被调含 "CalcMod for " + supply_factor 误用警告 country.cpp); ② `(tag_idx + gs 小时) % 24 == 0` 错峰每日脉冲 → 确定性 random_int (random.cpp:258) → 构造 CEventScope → on_action ×2 (通用槽 sub_140162FB0 单例+144 + 国家专属槽 `*(gs+312)+24×tag_idx`, onaction.cpp 启动期门) — **on_daily 派发** (§4.2.9); ③ cc+4880 72B/项桶数组 (计数 +4892) 逐桶到期处理 (24B 条 {+16 tag, +20 到期小时}; 桶身份未决); ④ cc+4744 convoys 派生值缓存重算 |
